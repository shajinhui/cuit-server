...
浏览器不直接访问学校教务认证系统。后端为每位用户创建独立的 `jwxt.Client` 和 `CookieJar`，避免不同学生的学校会话相互混用。校园跑轨迹和请求体由浏览器计算，Cloudflare Worker 持有 `APP_SECRET`、在 D1 中加密 token 和自动续登凭据，并完成全部上游请求；Go 后端只保存不透明 `sessionKey`、编排无人值守定时任务，并通过 HMAC 内部接口调用 Worker。
...
本项目为专有软件，著作权归作者所有，仓库公开仅代表允许浏览，不授予任何
使用许可。未经作者事先书面许可，不得复制、下载、修改、演绎、分发、部署、
转授权或用于任何商业目的；最终用户可以安装并使用作者官方发布的客户端
（PWA、Android APK）用于个人非商业用途。
