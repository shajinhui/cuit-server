...
服务端使用 Redis 缓存可以重新获取的查询结果，并在同一进程内合并同时发生的
同 Key 冷缓存请求。所有 JWXT 接口即使缓存命中也会先验证
`campus_session`，不会绕过现有会话权限。
...
| 计划完成情况 | 当前用户 | 1 小时 |
| 成绩 | 当前用户、指定学期 | 10 分钟 |
| 考试安排 | 当前用户、学期和考试类型 | 30 分钟 |
| 个人课表 | 当前用户、指定学期 | 6 小时 |
| 教室选项 | 指定学期和校区，全局共享 | 24 小时 |
| 教室占用快照 | 指定学期和校区，全局共享 | 24 小时 |
